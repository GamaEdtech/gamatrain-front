<template>
  <v-container id="post-list-page">
    <!-- Featued post section -->
    <v-row
      id="desktop-featured-post"
    ...
    >
      ...
      >
        ...
        <v-card
          ...
          flat
          :to="`/post/${featuredItems[slideIndex].id}/${featuredItems[0].slug}`"
          class="ma-1"
        >
          <v-card>
            <!-- <NuxtImg
      ...
      >
        ...
        <div
          ...
          class="featured-item"
        >
          <nuxt-link :to="`/post/${item.id}/${item.slug}`">
    ...
    <div id="post-list-section">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div class="subtitle-card-post text-h6 pl-2 font-weight-bold">
                <span
      ...
      <div
        v-else-if="postList.length == 0 && postLoading == false"
        class="text-center"
      >
...
// SEO
useHead({
  title: 'Exploring the Latest Insights in K12 Education | GamaTrain Post',
  meta: [
    {
      name: 'apple-mobile-web-app-title',
      content:
        'Exploring the Latest Insights in K12 Education | GamaTrain Post',
    },
    {
      name: 'og:title',
      content:
    ...
    },
    {
      name: 'description',
      content:
        'Stay informed and inspired with GamaTrain\'s insightful post, featuring thought-provoking articles on the latest trends and best practices in K12 education.',
    },
    {
      name: 'og:description',
      content:
...
// post serch section End

// category section start
const { data: postCategoriesResponse } = await useAsyncData('categories', () =>
  useApiService.get('/api/v2/tags/Post'),
)

const categoryValue = ref(route.query.cat ? Number(route.query.cat) : 0)
const postCategories = ref(postCategoriesResponse.value?.data || [])
...
const { data: initialPosts, pending: loadingPostsServer } = await useAsyncData(
  'postListSSR',
  () => {
    const params = {
...
const postLoading = ref(loadingPostsServer.value)

const processPosts = (list) => {
...
if (initialPosts.value?.data?.list) {
  postList.value = processPosts(initialPosts.value.data.list)
  pageCount.value = Math.ceil(
    initialPosts.value.data.totalRecordsCount / pageSize.value,
  )
}
