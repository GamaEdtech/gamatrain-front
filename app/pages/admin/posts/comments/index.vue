<template>
  <div class="w-100 h-100 d-flex flex-column align-start justify-start">
    ...
    <admin-common-modal
      ...
      title="Search"
    >
      <admin-posts-comments-modal-search
        :data="searchFilter"
        @search="startSearch"
      />
    </admin-common-modal>
    ...
    <admin-common-modal
      ...
      title="Detail"
    >
      <admin-posts-comments-modal-detail
...
const { loadingGetData: loading, data: list, getData, totalCount, pageCount } = usePostCommentAdmin()

const headers: DataTableHeader<CommnetPostAdminDTO>[] = [
  ...
  },
  {
    ...
    getText: (item: CommnetPostAdminDTO) => item.postId ? String(item.postId) : '-',
  },
  {
    title: 'Status',
    key: 'status',
    ...
    width: '20vw',
    type: 'chip',
    getChipColor: (item: CommnetPostAdminDTO) => getColorStatus(item.status),
  },
  {
    title: 'Action',
    key: 'Action',
    ...
    actions: [
      {
        ...
        show: (item: CommnetPostAdminDTO) => Boolean(item.postId),
        href: (item: CommnetPostAdminDTO) => `/post/${item.postId}`,
        target: '_blank',
      },
      {
        icon: 'md:settings',
        tooltip: 'More',
        onClick: (item: CommnetPostAdminDTO) => openModalMoreAction(item),
      },
    ],
  },
]
...
const searchFilter = reactive<CommentPostAdminSearchFilter>({
  startDate: '',
  endDate: '',
  commenterEmail: '',
  commenterName: '',
...
const openModalMoreAction = (item: CommnetPostAdminDTO) => {
...
const startSearch = async (item: CommentPostAdminSearchFilter) => {
...
const getColorStatus = (status?: CommentPostStatus) => {
  switch (status) {
    case 'Draft':
      return 'info'
    case 'Review':
