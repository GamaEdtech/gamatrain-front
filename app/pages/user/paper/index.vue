  ...
  <div class="w-100 h-100 d-flex flex-column align-start justify-start px-2 pa-md-0">
    ...
    <div class="w-100 d-flex justify-start align-center mt-4">
      ...
      >
        ...
        <span class="text-grey800 font-weight-bold text-h6">New Paper</span>
      </v-btn>
    </div>
...
import type { DataTableHeader, PastPaperBriefDTO } from '@/types'
...
const openModalDelete = (item: PastPaperBriefDTO) => {
  selectedItemIdForDelete.value = item.id.toString()
  showDeleteModal.value = true
}

const headers: DataTableHeader<PastPaperBriefDTO>[] = [
  { title: 'ID', key: 'id', sortable: false, width: '10vw' },
  {
    ...
    width: '30vw',
    type: 'link',
    getTo: (item: PastPaperBriefDTO) => `/paper/${item.id}`,
  },
  ...
  },
  {
    ...
    width: '15vw',
    type: 'chip',
    getText: (item: PastPaperBriefDTO) => getStatusTitle(item.status),
    getChipColor: (item: PastPaperBriefDTO) => getStatusColor(item.status),
  },
  {
    title: 'Action',
    key: 'Action',
    ...
    actions: [
      {
        icon: 'md:visibility',
        tooltip: 'View',
        to: (item: PastPaperBriefDTO) => `/paper/${item.id}`,
      },
      {
        icon: 'md:edit',
        tooltip: 'Edit',
        to: (item: PastPaperBriefDTO) => `/user/paper/edit/${item.id}`,
      },
      {
        icon: 'md:delete',
        tooltip: 'Delete',
        onClick: (item: PastPaperBriefDTO) => openModalDelete(item),
      },
    ],
  },
]
