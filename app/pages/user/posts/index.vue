    ...
    <div class="w-100 d-flex justify-space-between align-center ga-2 flex-wrap mt-4">
      <div class="d-flex align-center ga-2 flex-wrap">
        ...
        >
          ...
          <span class="text-grey800 font-weight-bold text-h6">New Post</span>
        </v-btn>
...
import type { PostUserBreifDTO, PostUserBreifDTOStatus, DataTableHeader } from '@/types'
import { USER_POST_STATUS } from '@/constants'
...
const headers: DataTableHeader<PostUserBreifDTO>[] = [
  ...
  },
  {
    ...
    width: '15vw',
    type: 'chip',
    getChipColor: (item: PostUserBreifDTO) => getStatusColor(item.status),
  },
  {
    title: 'Action',
    key: 'Action',
    ...
    actions: [
      {
        icon: 'md:visibility',
        tooltip: 'View',
        to: (item: PostUserBreifDTO) => `/post/${item.id}`,
        target: '_blank',
      },
      {
        icon: 'md:edit',
        tooltip: 'Edit',
        to: (item: PostUserBreifDTO) => `/user/posts/edit/${item.id}?fromPage=${page.value}`,
      },
    ],
  },
]
...
const statusFilterItems = [
  { id: '', title: 'All' },
  ...USER_POST_STATUS.map(item => ({ id: item, title: item })),
]
...
const pendingStatus = ref<PostUserBreifDTOStatus | ''>('')
...
// const openModalDelete = (item: PostUserBreifDTO) => {
...
const getStatusColor = (status: PostUserBreifDTOStatus) => {
  switch (status) {
    case 'Confirmed':
      return 'success'
    case 'Review':
