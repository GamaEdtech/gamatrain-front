import type {
  ...
  ResponseListDTO,
  CommnetPostAdminDTO,
  CommnetPostDetailAdminDTO,
  GetCommentPostAdminParams,
} from '@/types'

const data = ref<CommnetPostAdminDTO[]>([])
...
export const usePostCommentAdmin = () => {
  const { $toast } = useNuxtApp()
  const { handleApiResponseError, handleApiCatchError, createApiFailure } = useApiErrorHandler()

  const getData = async (params: GetCommentPostAdminParams) => {
    ...
    try {
      ...
      const response = await useApiService.get<
        ApiResult<ResponseListDTO<CommnetPostAdminDTO>>
      ...
      return createApiFailure<ResponseListDTO<CommnetPostAdminDTO>>(err)
    }
    finally {
  ...
  const getItemById = async (id: string) => {
    ...
    try {
      ...
      if (!item) {
        return { succeeded: false, data: undefined, errors: [] } as unknown as ApiResult<CommnetPostDetailAdminDTO>
      }
      return {
        succeeded: true,
        data: { id: item.id, postId: item.postId, postTitle: item.postTitle, comment: item.comment },
      } as unknown as ApiResult<CommnetPostDetailAdminDTO>
    }
    finally {
