...
export const usePostAdmin = () => {
  ...
  const getData = async (params: GetAdminPostParams) => {
    loadingGetData.value = true

    try {
      const response = await useApiService.get<
        ApiResult<ResponseListDTO<AdminPostDTO>>
      >(
        '/api/v2/admin/posts',
        {
      ...
      return createApiFailure<ResponseListDTO<AdminPostDTO>>(err)
    }
    finally {
  ...
  const getItemById = async (id: string | number) => {
    ...
    try {
      const response = await useApiService.get<ApiResult<AdminPostDetailDTO>>(
      ...
      return createApiFailure<AdminPostDetailDTO>(err)
    }
    finally {
